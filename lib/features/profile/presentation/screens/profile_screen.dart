...
class _ProfileTabState extends State<ProfileTab> {
              ...
              child: Column(
                ...
                children: [
                  Padding(
                    ...
                    child: Text(_controller.errorMessage, style: AppTextStyles.bodyNormal, textAlign: TextAlign.center),
                  ),
                ...
                Padding(
                  ...
                  child: Column(
                    ...
                    children: [
                      ...
                      Center(child: Text(p['fullname'] ?? 'Học viên', style: AppTextStyles.heading1)),
                      const SizedBox(height: 4),
                      Center(child: Text(p['email'] ?? '', style: AppTextStyles.bodyNormal.copyWith(color: Colors.grey))),
                      const SizedBox(height: 24),
                      _buildStatsRow(p),
                      const SizedBox(height: 32),
                      _buildSectionTitle('Thông tin học tập', Icons.bar_chart_rounded, AppColors.primary),
                      const SizedBox(height: 16),
  ...
  Widget _buildHeader(String avatarUrl) {
    return Container(
      width: double.infinity, height: 180, color: Colors.transparent,
      child: Stack(
        alignment: Alignment.center,
        children: [
          Positioned(top: 40, right: 16, child: IconButton(icon: const Icon(Icons.settings_outlined, color: Colors.blueAccent, size: 26), onPressed: () {})),
          Positioned(bottom: 0, child: Container(
            ...
            child: ClipOval(child: Image.network(avatarUrl, width: 110, height: 110, fit: BoxFit.cover, errorBuilder: (_, __, ___) => const Icon(Icons.person, size: 50))),
          )),
        ],
      ),
    );
  ...
  Widget _buildStatsRow(Map<String, dynamic> p) {
    Widget statCol(String emoji, String count, String label) => Column(children: [
  ...
  Widget _buildSectionTitle(String title, IconData icon, Color color) {
    return Row(children: [
      ...
      const SizedBox(width: 8),
      Text(title, style: AppTextStyles.heading1.copyWith(fontSize: 20)),
    ]);
  }
    ...
    Widget card(String icon, String title, String subtitle) => Container(
      ...
      child: Row(children: [
        ...
        Expanded(child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
          Text(title, style: const TextStyle(fontSize: 17, fontWeight: FontWeight.w800, color: AppColors.primary)),
